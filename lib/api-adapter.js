...
export const UserScripts = {
	...
	async register(scripts) {
		...
		} else if (typeof chrome !== 'undefined' && chrome.userScripts) {
			// Chrome Implementation
			// Ensure we are using the Promise-based API if available, or callback wrap
			if (chrome.userScripts.register.length === 0 || typeof Promise !== 'undefined') {
				// Try promise based (modern MV3)
				try {
				...
				// Fallback for older callback style if needed (unlikely for userScripts API)
	...
	async unregister(scriptIds) {
		if (typeof browser !== 'undefined' && browser.userScripts) {
			...
			// Chrome
			try {
				// Check if unregister is promise based
				await chrome.userScripts.unregister({ ids: scriptIds });
			} catch (e) {
